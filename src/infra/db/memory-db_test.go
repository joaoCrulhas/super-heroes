...
import (
	...
	db "github.com/joaoCrulhas/omnevo-super-heroes/src/infra/db"
...
func (suite *MemoryDbTestSuite) TestShouldReturnHerosThatMatchSuperPower() {
	fmt.Println("From TestExample")
	expected := getSuperHeroes()[0:1]
	got, _ := suite.sut.GetBySuperPower(suite.ctx, []string{"flight"})
	suite.Equal(expected, got)
}

func (suite *MemoryDbTestSuite) TestShouldReturnAnEmptyArrayIfNoSuperHeroWithSuperPower() {
	fmt.Println("From TestExample")
	got, _ := suite.sut.GetBySuperPower(suite.ctx, []string{"random_power"})
